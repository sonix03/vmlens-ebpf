...
function applyLiveFlow(graph: GraphData, flow: Flow, observedAt: string): GraphData {
  ...
  const edge: GraphEdge = {
    ...
    retransmission_count: (previous?.retransmission_count ?? 0) + (flow.retransmission_count ?? 0),
    avg_rtt_ms: flow.avg_rtt_ms && flow.avg_rtt_ms > 0 ? flow.avg_rtt_ms : previous?.avg_rtt_ms,
    avg_app_delay_ms: flow.avg_app_delay_ms && flow.avg_app_delay_ms > 0 ? flow.avg_app_delay_ms : previous?.avg_app_delay_ms,
