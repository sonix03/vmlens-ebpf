...
func Routes(handlers *Handlers, hub *realtime.Hub, allowedOrigins []string) http.Handler {
	...
	mux.HandleFunc("GET /api/connections", handlers.ListConnections)
	...
	mux.HandleFunc("GET /api/cloud/status", handlers.CloudStatus)
	mux.HandleFunc("GET /api/cloud/context", handlers.CloudContext)
