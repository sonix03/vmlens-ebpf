...
func (s *StatsService) Summary(ctx context.Context) (model.Summary, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT
			...
			(SELECT COUNT(*) FROM network_flows WHERE scope IN ('external_public', 'external_private')),
			...
			COALESCE((SELECT SUM(bytes_sent + bytes_received) FROM network_flows WHERE scope IN ('external_public', 'external_private')), 0),
			COALESCE((SELECT SUM(bytes_sent) FROM network_flows WHERE scope IN ('external_public', 'external_private')), 0),
			COALESCE((SELECT SUM(bytes_received) FROM network_flows WHERE scope IN ('external_public', 'external_private')), 0),
