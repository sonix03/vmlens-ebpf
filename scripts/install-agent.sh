...
if [[ ${EUID} -ne 0 ]]; then
  echo "Run with sudo: sudo BACKEND_URL=http://127.0.0.1:18080 bash scripts/install-agent.sh" >&2
  exit 1
fi
...
CONNECTIVITY_PROBE_ENABLED="${CONNECTIVITY_PROBE_ENABLED:-true}"
CONNECTIVITY_PROBE_INTERVAL="${CONNECTIVITY_PROBE_INTERVAL:-5s}"
CONNECTIVITY_PROBE_TIMEOUT="${CONNECTIVITY_PROBE_TIMEOUT:-1s}"
CONNECTIVITY_PROBE_LISTEN_ADDR="${CONNECTIVITY_PROBE_LISTEN_ADDR:-0.0.0.0:18081}"
...
auto_filter_tunnel_peer() {
  ...
  echo "Agent installer: filtering SSH peer from captured flows: ${peer_ip}" >&2
}
...
resolve_capture_interface() {
  ...
  if [[ -n "${fallback}" ]]; then
    if [[ -n "${CAPTURE_INTERFACE}" ]]; then
      echo "Agent installer: capture interface ${CAPTURE_INTERFACE} not found; using ${fallback}" >&2
    fi
...
CONNECTIVITY_PROBE_ENABLED=${CONNECTIVITY_PROBE_ENABLED}
CONNECTIVITY_PROBE_INTERVAL=${CONNECTIVITY_PROBE_INTERVAL}
CONNECTIVITY_PROBE_TIMEOUT=${CONNECTIVITY_PROBE_TIMEOUT}
CONNECTIVITY_PROBE_LISTEN_ADDR=${CONNECTIVITY_PROBE_LISTEN_ADDR}
TENANT_ID=${TENANT_ID}
AGENT_PRIVATE_IPS=${AGENT_PRIVATE_IPS}
AGENT_PUBLIC_IP=${AGENT_PUBLIC_IP}
...
[Unit]
Description=VMLens TC/eBPF network tracker
...
echo "TC/eBPF tracker installed; logs: journalctl -u vmlens-agent -f"
